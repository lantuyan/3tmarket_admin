...
exports.createCollector = (req, res) => {
    ...
    users.createArgon2User(userId, email, password)
        .then(response_create => {
            ...
            databases.createDocument(process.env.APPWRITE_DB, process.env.APPWRITE_USER_COLLECTION, userIdDB, { email, role: "collector", name, phonenumber, zalonumber, address, uid: userIdDB });
        }).catch(error => {
